...
class BrowserResolver:
    def __init__(self, **kwargs):
        ...
        keys_env = os.getenv("SCRAPER_API_KEYS", os.getenv("SCRAPER_API_KEY", "")).strip()
    ...
    def _rotate_key(self):
        if len(self.api_keys) > 1:
            ...
            log.warning("scraper_api_key_rotated", extra={"new_index": self.current_key_index})

    async def start(self) -> None:
        if not self.api_keys:
            log.error("Nenhuma SCRAPER_API_KEY configurada!")
        log.info("scraper_api_resolver_started", extra={"total_keys": len(self.api_keys)})
    ...
    async def resolve(self, url: str) -> ResolvedItem | None:
        ...
        max_attempts = len(self.api_keys) # Tenta no máximo o número de chaves que você tem
